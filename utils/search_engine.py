...
class SearchEngine:
    ...
    #         size_b = None
    #         try:
    #             size_b = int(it.get("size")) if it.get("size") is not None else None
    #         except Exception:
    #             size_b = None
    ...
    #         modified_raw = it.get('date_modified') or None
    #         modified = None
    #         if modified_raw:
    #             try:
    #                 # Everything sometimes returns FILETIME numeric
    #                 if str(modified_raw).isdigit():
    #                     ft = int(modified_raw)
    #                     modified = datetime.fromtimestamp((ft - 116444736000000000) / 10_000_000).strftime("%Y-%m-%d %H:%M:%S")
    #                 else:
